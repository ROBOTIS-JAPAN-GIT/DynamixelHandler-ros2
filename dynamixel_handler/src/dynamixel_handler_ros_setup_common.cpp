...
void DynamixelHandler::SetupRosInterfaces(bool no_use_command_line) {
    ...
    if (use_["x"])   SetupRosInterfacesX(no_use_command_line);
    if (use_["p"])   SetupRosInterfacesP(no_use_command_line);
    if (use_["pro"]) SetupRosInterfacesPro(no_use_command_line);
